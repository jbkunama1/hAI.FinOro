...
# ── In-Memory-Log ──────────────────────────────────────────────────────────────
_log_buf: deque = deque(maxlen=MAX_LOG)
...
# ── Hilfsfunktion API GET ──────────────────────────────────────────────────────
def get(path: str, params: Optional[dict] = None) -> Optional[requests.Response]:
    headers = get_headers()
    if headers is None:
        return None
    try:
...
def get_price(instrument_id: int) -> Optional[str]:
    """
    eToro Public API v1 Preis-Endpunkte (mehrere Fallbacks).
    """
    # Versuch 1: market-data Endpunkt
    r = get(f"/market-data/instruments/{instrument_id}/prices/last")
    if r and r.status_code == 200:
        d = r.json()
        val = (d.get("last") or d.get("price") or d.get("ask")
               or d.get("bid") or d.get("close"))
        if val is not None:
            return str(val)
    # Versuch 2: rates Endpunkt
    r2 = get(f"/instruments/{instrument_id}/rates")
    if r2 and r2.status_code == 200:
        d2 = r2.json()
        if isinstance(d2, list) and d2:
            d2 = d2[0]
        val2 = (d2.get("last") or d2.get("rate") or d2.get("ask") or d2.get("price"))
        if val2 is not None:
            return str(val2)
    # Versuch 3: allgemeiner instruments Endpunkt
    r3 = get(f"/instruments/{instrument_id}")
    if r3 and r3.status_code == 200:
        d3 = r3.json()
        val3 = (d3.get("lastPrice") or d3.get("price") or d3.get("rate") or d3.get("ask"))
        if val3 is not None:
            return str(val3)
    return None
...
# ── Instrument-Suche Symbol ODER Name ─────────────────────────────────────────
def search_instrument(query: str) -> tuple:
    """
    Sucht Instrument auf eToro Public API v1.
    Probiert mehrere bekannte Endpunkte.
    Gibt (list[dict], error:str|None) zurück.
    """
    headers = get_headers()
    if headers is None:
    ...
    for url, params in endpoints:
        try:
            kw: dict = dict(headers=headers, timeout=TIMEOUT_API)
            if params:
                kw["params"] = params
            r = http.get(url, **kw)
            if r.status_code == 200:
                ...
                if isinstance(data, list):
                    ...
                    items = (data.get("instruments") or data.get("items")
                ...
                for item in items:
                    iid  = (item.get("instrumentId") or item.get("InstrumentId")
                            or item.get("id"))
                    sym  = (item.get("internalSymbol") or item.get("symbol")
                            or item.get("ticker") or "?")
                    name = (item.get("displayName") or item.get("displayname")
                            or item.get("name") or "?")
                    cls  = (item.get("assetClass") or item.get("instrumentType")
                            or item.get("type") or "?")
                    if iid and not any(x["id"] == iid for x in results):
                        results.append({"id": iid, "symbol": sym, "name": name, "class": cls})
                if results:
...
def resolve_instrument_ids() -> tuple:
    ...
    for inst in INSTRUMENTS_TO_TRACK:
        ...
        if cached:
            ids[sym] = int(cached)
            continue
        # 2. Aus DEFAULT_CONFIG (kein API-Call nötig)
        ...
        if default_id:
            ...
            continue
        # 3. API-Suche (nur wenn kein Fallback vorhanden)
        results, err = search_instrument(inst["search"])
        if err:
            errors.append(f"{sym}: {err}")
        elif results:
            ids[sym] = results[0]["id"]
            cfg[cfg_key] = results[0]["id"]
            save_config(cfg)
...
def get_llm_signal(context: dict) -> str:
    ...
    prompt = (f"Du bist ein Trading-Assistent. Analysiere:\n{json.dumps(context, indent=2)}\n"
...
def agent_tick() -> None:
    ...
    if mode == "trade" and signal.startswith("BUY"):
        iid    = ids.get("BTC")
...
def index():
    ...
    # Warnmeldung falls Keys fehlen oder IDs nicht auflösbar
    if not cfg.get("API_KEY", "").strip() or not cfg.get("USER_KEY", "").strip():
        cfg_warn = '<div class="error-msg">⚠️ <strong>API-Key oder User-Key fehlt.</strong> Bitte unter <a href="/config" style="color:var(--red);text-decoration:underline;">Config</a> eintragen.</div>'
    elif errs:
    ...
    kpis = "".join(
        ...
        f'<div style="font-size:18px;font-weight:700;color:var(--green);">{prices.get(i["symbol"],"–")}</div>'
        f'</div>'
...
def agent():
    ...
    log_html = "\n".join(
        f'<div class="log-entry">{e}</div>' for e in list(_log_buf)[:20]
...
def config():
    ...
    if request.method == "POST":
        try:
            ...
            cfg.update({
                ...
                "API_URL":      request.form.get("API_URL",  "https://public-api.etoro.com/api/v1").strip().rstrip("/"),
                "BASE_URL":     request.form.get("BASE_URL", "https://api.etoro.com").strip().rstrip("/"),
                "SANDBOX":      request.form.get("SANDBOX") == "1",
                # LLM
                "LLM_BASE_URL": request.form.get("LLM_BASE_URL", "").strip().rstrip("/"),
                "LLM_URL":      request.form.get("LLM_BASE_URL", "").strip().rstrip("/"),
    ...
    <form method="post">
      <div class="card" style="margin-bottom:16px;">
        ...
        <div class="form-row">
          <div class="form-group"><label>API Key</label><input type="password" name="API_KEY" value="{v("API_KEY")}" placeholder="Dein API-Key"></div>
          <div class="form-group"><label>User Key (JWT)</label><input type="password" name="USER_KEY" value="{v("USER_KEY")}" placeholder="eyJ..."></div>
        </div>
        <div class="form-row">
          <div class="form-group"><label>API URL</label><input name="API_URL" value="{v("API_URL","https://public-api.etoro.com/api/v1")}" placeholder="https://public-api.etoro.com/api/v1"></div>
      ...
      <div class="card" style="margin-bottom:16px;">
        ...
        <div class="form-row">
          <div class="form-group"><label>LLM Base URL</label><input name="LLM_BASE_URL" value="{v("LLM_BASE_URL","https://9router.arbeitermili.eu/v1")}" placeholder="https://...api.../v1"></div>
          <div class="form-group"><label>LLM Modell</label><input name="LLM_MODEL" value="{v("LLM_MODEL","finance")}" placeholder="finance"></div>
        </div>
        <div class="form-row">
          <div class="form-group"><label>LLM API Key</label><input type="password" name="LLM_API_KEY" value="{v("LLM_API_KEY")}" placeholder="sk-..."></div>
...
def debug_instruments():
    ...
    err_html  = f'<div class="error-msg">{"<br>".join(errors)}</div>' if errors else ""
    ...
    <div class="card" style="margin-bottom:20px;">
      ...
      {"<div style='font-size:12px;color:var(--muted);margin-bottom:12px;'>Ergebnisse für <strong style='color:#a5b4fc'>" + query + "</strong></div>" if query else ""}
...
def debug_log():
    ...
    log_html = "".join(
        f'<div class="log-entry">{e}</div>' for e in log_entries
...
def debug_order():
    ...
    if request.method == "POST":
        mode = cfg.get("MODE", "observe")
        if mode != "trade":
            result_html = '<div class="error-msg">Modus ist <strong>observe</strong> – Orders deaktiviert. Wechsle in Config auf <strong>trade</strong>.</div>'
        else:
    ...
    <div class="page-header">
      <h2>Order-Test</h2>
      <p>Nur im Modus <strong>trade</strong> aktiv · Aktuell: <span class="badge badge-{mode_badge}">{cfg.get("MODE","observe")}</span></p>
    </div>
    {result_html}
    <form method="post">
      <div class="card">
